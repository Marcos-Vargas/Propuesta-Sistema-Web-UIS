...
if(isset($_GET['id'])) {
	...
	$query = mysqli_query($connect,"SELECT * FROM noticias WHERE id = '".$_GET['id']."'");
	
	while($row=mysqli_fetch_array($query)) { 
	
	$visitas = mysqli_query($connect,"UPDATE noticias SET visitas = visitas + 1 WHERE id = '".$_GET['id']."'");
	
	$usuarios = mysqli_query($connect,"SELECT * FROM usuario WHERE id = '".$row['reportero']."'");		
...
while($com=mysqli_fetch_array($coment)) {
	...
	$usuar = mysqli_query($connect,"SELECT * FROM usuario WHERE id = '".$com['usuario']."'");
	$us = mysqli_fetch_array($usuar);
?>
...
$insert = mysqli_query($connect,"INSERT INTO comentarios (comentario,not_id,usuario) values ('".$_POST['comentario']."','".$_GET['id']."','".$_SESSION['id']."')");
