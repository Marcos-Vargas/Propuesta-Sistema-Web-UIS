...
if(isset($_GET['id'])) {
	...
	$rang = mysqli_fetch_array($rango);
	
	$rank = mysqli_query($connect,"SELECT * FROM rango WHERE id != '".$rang['id']."'");
?>
  ...
  <p>
    Rango: 
      ...
      <select name="rango" id="select">
        ...
        <?php while($rangos=mysqli_fetch_array($rank)) { ?>
...
<?php if(isset($_POST['editar'])) {
	...
	$sql = mysqli_query($connect,"UPDATE usuario SET usuario = '".$usuario."', contrasena = '".$contrasena."', rango = '".$ranki."', avatar = '".$destino1."' WHERE id = '".$_GET['id']."'");
